            ...
            while ( ($data = fgetcsv ( $handle, 0, ',' )) !== FALSE ) {
                if($once){ $once = false; continue; }
                    try{
                        ...
                        $course_data->DISTANCE    = $data[6]  === ""?null:$data[6];    
                        $course_data->EMPLOYURL   = $data[7];                   
                        $course_data->EMPLOYURLW  = $data[8];
                        $course_data->FOUNDATION  = $data[9]  === ""?null:$data[9];
                        $course_data->HONOURS     = $data[10] === ""?null:$data[10];
                        // $course_data->JACS
                        $course_data->KISCOURSEID = $data[14];
                        $course_data->KISMODE     = $data[15] === ""?null:$data[15];
                        // $course_data->LDCS
                        $course_data->LOCCHNGE    = $data[19] === ""?null:$data[19];
                        $course_data->LTURL       = $data[20];
                        $course_data->LTURLW      = $data[21];
                        $course_data->NHS         = $data[22] === ""?null:$data[22];
                        $course_data->NUMSTAGE    = $data[23] === ""?null:$data[23];
                        $course_data->SANDWHICH   = $data[24] === ""?null:$data[24];
                        $course_data->SUPPORTURL  = $data[25];
                        $course_data->SUPPORTURLW = $data[26];
                        $course_data->TITLE       = $data[27];
                        $course_data->TITLEW      = $data[28];
                        $course_data->UCASPROGID  = $data[29] === ""?null:$data[29];
                        $course_data->UKPRNAPPLY  = $data[30] === ""?null:$data[30];
                        $course_data->YEARABROAD  = $data[31] === ""?null:$data[31];
                        $course_data->KISAIM      = $data[32];

                        $course_data->save();
                        echo $course_data->UKPR;
                    }
